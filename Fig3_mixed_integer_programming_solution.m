for seedd = 1:30
    ...
    %(7.25)-(7.26) in "Foundations of User-Centric Cell-Free Massive MIMO"
    %book
    [signal_P_MMSE, signal2_P_MMSE, scaling_P_MMSE,...
        ...
        functionComputeExpectations(Hhat,H,D_all,C,nbrOfRealizations,N,K,L,p_full);
    ...
    for k = 1:K
        ...
        %Compute the vector in (7.25) for UE k
    ...
    for sss = 1:numberSetup
        ...
        variable rho(K*L,1)
        minimize Pdisp/sigmaCool + PPl*sum(zz) + DeltaTr*quad_form(rho,eye(K*L))/10 ...
        ...
        TotalPower(1,sss) = Pdisp/sigmaCool + PPl*sum(zz) + DeltaTr*quad_form(rho,eye(K*L))/10 ...
            + POLT*(1:W)*ll/sigmaCool + Pproc0*(1:W)*dd/sigmaCool ...
            + DeltaProc/GOPSmax*XLP*sum(sum(xx))/sigmaCool ...
            + DeltaProc/GOPSmax*FLP/sigmaCool;
        ...
        TotalPower(2,sss) = Pdisp/sigmaCool + PPl*sum(zz) + DeltaTr*quad_form(rho,eye(K*L))/10 ...
            + POLT*(1:W)*ll/sigmaCool + Pproc0*(1:W)*dd/sigmaCool ...
            + DeltaProc/GOPSmax*XLP*sum(sum(xx))/sigmaCool ...
            + DeltaProc/GOPSmax*FLP/sigmaCool;
